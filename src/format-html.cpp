...
std::unique_ptr<DOM> extract_html(State& state, std::unique_ptr<icu::UnicodeString> data) {
	...
	bool insert_heading_marker = true; // TODO cli arg?
	if(insert_heading_marker) {
		// For apertium -H (github issue#15):
		dom->tags_heading = make_xmlChars("h1", "h2", "h3", "h4", "h5", "h6", "title");
	}
	dom->save_spaces();
